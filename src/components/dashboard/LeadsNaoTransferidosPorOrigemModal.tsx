...
interface LeadsNaoTransferidosPorOrigemModalProps {
  isOpen: boolean;
  onClose: () => void;
  loading?: boolean;
...
export const LeadsNaoTransferidosPorOrigemModal: React.FC<
  LeadsNaoTransferidosPorOrigemModalProps
> = ({ isOpen, onClose, loading = false, currentFilters }) => {
  ...
  useEffect(() => {
    ...
    const fetchTotais = async () => {
      try {
        const lista = await httpClient.getOrigensTotais({
          ...(currentFilters || {}),
          transferido: false,
        });
        setTotaisPorOrigem(lista || []);
      } catch (e) {
        setTotaisPorOrigem([]);
      }
    };
  ...
  const leadsAtuais = origemSelecionada
    ? leadsPorOrigem[origemSelecionada]
    : null;
      ...
      <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto border-0 shadow-2xl">
        <DialogHeader className="bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-t-lg -m-6 mb-6 p-6">
          <DialogTitle className="text-xl font-semibold flex items-center gap-2">
            ...
            Leads Não Transferidos por Origem
          </DialogTitle>
                ...
                <>
                  <div className="rounded-lg border border-blue-200 overflow-hidden shadow-sm">
                    <Table>
                      ...
                      <TableBody>
                        {leadsAtuais.dados.map(
                          (lead: ILeadTotal, index: number) => (
                            <TableRow
                              key={lead.id}
            ...
            <div className="border-t pt-6">
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">
                  Clique em uma das origens acima para visualizar os leads não
